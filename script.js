...
async function loadPropertyOverrides() {
  try {
    const snap = await withPropertyTimeout(db.collection('property_settings').get(), 6000);
    snap.forEach(doc => {
      const data = doc.data();
    ...
    // Always render — with real photos if the fetch succeeded, or with
    // defaults if it failed/timed out, so the skeleton never gets stuck.
...
// ---- Skeleton loader shown while real property photos are fetched ----
// Avoids briefly showing Unsplash stock placeholders before real photos arrive.
function renderPropertySkeletons(count = 6) {
  const grid = document.getElementById('accommodations-grid');
  if (!grid) return;
  if (!document.getElementById('property-skeleton-style')) {
    const style = document.createElement('style');
    style.id = 'property-skeleton-style';
    style.textContent = `
      @keyframes propertySkeletonPulse { 0% { opacity: 1; } 50% { opacity: .55; } 100% { opacity: 1; } }
      .property-skeleton-img, .property-skeleton-line {
        background: #e2e2e2;
        border-radius: 6px;
        animation: propertySkeletonPulse 1.4s ease-in-out infinite;
      }
    `;
  ...
  grid.innerHTML = Array.from({ length: count }).map(() => `
    <article class="accommodation-card">
      <div class="card-image">
        <div class="property-skeleton-img" style="width:100%;height:100%;"></div>
      </div>
      <div class="card-content">
        <div class="property-skeleton-line" style="height:20px;width:70%;margin-bottom:10px;"></div>
        <div class="property-skeleton-line" style="height:14px;width:50%;margin-bottom:10px;"></div>
        <div class="property-skeleton-line" style="height:14px;width:90%;margin-bottom:6px;"></div>
        <div class="property-skeleton-line" style="height:14px;width:80%;margin-bottom:14px;"></div>
        <div class="property-skeleton-line" style="height:32px;width:100%;"></div>
      </div>
    </article>
  `).join('');
}

// ---- Timeout helper so a slow/offline Firestore never leaves the skeleton stuck ----
function withPropertyTimeout(promise, milliseconds) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error('Property data timed out')), milliseconds))
  ]);
}
